from pyspark.sql.functions import (
    struct, lit, col, array, when, isnull, expr, current_timestamp, date_format,
    collect_list
)
...
def get_contract(df):
    ...
    df_with_contract = df.withColumn("contractTitle", contract_title)
    contract_title_nl = expr("filter(contractTitle, x -> x is not null)")
    ...
    return df_with_contract.select(
        "account_id",
